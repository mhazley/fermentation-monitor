...
bool DS18::read() {
  ...
  if (!_wire.search(_addr)) {
    _searchDone = true;
    _wire.reset_search();
    Serial.println("Searched Failed");
    return false;
  }
  ...
  return start_conversion_and_read(_addr);
}

void DS18::start_conv(uint8_t addr[8]) {
#ifdef DEBUG_DS18
  Serial.println("DS18::start_conv");
  for( int i = 0; i < 8; i++ )
  {
...
bool DS18::start_conversion_and_read(uint8_t addr[8]) {
